...
switch($_REQUEST["mode"])
{
		case "auth":
				...
				$res = userAuth($_REQUEST["user"],$_REQUEST["pass"],$conn); 
				echo ("{\"Status\":". $res ."}");
			break;
		case "post":
				//echo $_REQUEST["field"]."|".$_REQUEST["value"];
...
function post_value_noret($field, $value, $connection){
	...
	$sql = "UPDATE `tbl_broker` SET `FIELD`='".$field."',`VALUE`='". $value ."',`TIMESTAMP`=CURRENT_TIMESTAMP WHERE `FIELD`='".$field."';";
